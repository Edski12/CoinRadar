const navItems = [
  { label: "Coins", href: "coins.php", key: "coins" },
  { label: "Watchlist", href: "watchlist.php", key: "watchlist" },
  { label: "Settings", href: "settings.php", key: "settings" },
];
...
function authLinks() {
  const user = window.COINRADAR_USER;
  if (!user)
    return (
      '<a class="btn btn-outline-dark btn-sm me-2" href="' +
      assetPath("login.php") +
      '">Sign in</a><a class="btn btn-primary btn-sm" href="' +
      assetPath("signup.php") +
      '">Sign up</a>'
    );
  return `<span class="small text-muted me-2">${user.name}</span><a class="btn btn-outline-dark btn-sm" href="${assetPath("auth/logout.php")}">Sign out</a>`;
}
...
export function renderNav() {
    ...
    mobileTarget.innerHTML = `
            <nav class="navbar navbar-light bg-light border-bottom d-lg-none">
                <div class="container-fluid">
                    <a class="navbar-brand d-flex align-items-center mb-0" href="${pageHref("index.php")}">
                    ...
                    <div class="d-flex align-items-center"><button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#sidebarMenu"
  ...
  if (sidebarTarget) {
    sidebarTarget.innerHTML = `
            <aside class="bg-light border-end p-4 min-vh-100 d-flex flex-column">
                <a href="${pageHref("index.php")}" class="d-flex align-items-center mb-4 text-dark text-decoration-none">
...
function activateCurrentNav() {
  const currentPage =
    window.location.pathname.split("/").pop().toLowerCase() || "index.php";
