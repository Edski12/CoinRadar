...
function renderTrending(payload) {
  ...
  trendingTopics.innerHTML = topics.length
    ? topics
        .map(
          (topic) =>
            `<span class="badge text-bg-light">${topic.name || topic.topic || topic.title || topic}</span>`,
        )
        .join("")
...
function renderNews(items) {
  if (!items.length) {
    newsList.innerHTML =
      '<div class="text-muted">No major headlines found.</div>';
    return;
  }
...
async function loadDashboard() {
  try {
    ...
    pulseGrid.innerHTML =
      '<div class="col-12 text-danger">Unable to load market pulse.</div>';
