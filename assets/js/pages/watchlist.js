...
async function renderCharts(symbols) {
  for (const symbol of symbols) {
    try {
      const closes = (await getKlines(symbol, "1h", 24)).map(
        (item) => item.close,
      );
...
function renderModalCoinList(filter = "") {
  ...
  if (!filtered.length) {
    coinListEl.innerHTML =
      '<div class="text-center text-muted py-3">No coins match your search.</div>';
    return;
  }

  coinListEl.innerHTML = filtered
    .map(
      (coin) => `
  ...
  coinListEl.querySelectorAll(".modal-coin-item").forEach((button) => {
    button.addEventListener("click", () =>
      selectCoinForAdd(button.dataset.symbol),
    );
  });
}
...
function renderWatchlistTable() {
  if (!watchlist.length) {
    tableBody.innerHTML =
      '<tr><td colspan="7" class="text-center text-muted">Your watchlist is empty. Click "+ Add Coin" to get started.</td></tr>';
  ...
  tableBody.querySelectorAll(".remove-coin-btn").forEach((button) => {
    button.addEventListener("click", async (event) => {
      ...
      await removeHolding(button.dataset.symbol);
  ...
  tableBody.querySelectorAll(".coin-row").forEach((row) => {
    row.addEventListener("click", (event) => {
      if (
        ...
        event.target.closest(".remove-coin-btn")
      )
        return;
...
async function refreshAll() {
  try {
    [availableCoins, watchlist] = await Promise.all([
      getAllTickers(),
      readWatchlist(),
    ]);
    renderWatchlistTable();
    ...
    tableBody.innerHTML =
      '<tr><td colspan="7" class="text-center text-danger">Unable to load watchlist prices.</td></tr>';
  }
}

document
  .getElementById("backToSearchBtn")
  .addEventListener("click", resetModalToSearch);
searchInput.addEventListener("input", () =>
  renderModalCoinList(searchInput.value),
);
confirmAddBtn.addEventListener("click", async () => {
  const amount = parseFloat(coinAmountInput.value);
  if (!selectedSymbol || !Number.isFinite(amount) || amount < 0) return;

  await upsertHolding(selectedSymbol, amount);
  watchlist = await readWatchlist();
  renderWatchlistTable();

  const modal = bootstrap.Modal.getInstance(
    document.getElementById("addCoinModal"),
  );
  modal?.hide();
});

document
  .getElementById("addCoinModal")
