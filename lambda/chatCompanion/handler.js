...
const OLLAMA_URL = (process.env.OLLAMA_URL || "http://127.0.0.1:11434").replace(
  /\/$/,
  "",
); //configure in lambda to work online
const OLLAMA_MODEL = process.env.OLLAMA_MODEL || "llama3.2:latest";
const OLLAMA_MAX_TOKENS = Number(process.env.OLLAMA_MAX_TOKENS || 300);
...
function compactAskAnswer(answer) {
  ...
  return (
    ...
    answer.text ||
    answer.summary ||
    JSON.stringify(answer).slice(0, 1000)
  );
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const newsQuery = symbols.length
      ? symbols.map((symbol) => symbol.replace(/USDT$/i, "")).join(" OR ")
      : body.message;
    const shouldSearchNews =
      wantsCurrentEvents(body.message) || Boolean(body.pageContext?.symbol);
    const [newsResult, askResult] = await Promise.allSettled([
      shouldSearchNews
        ? searchNews(newsQuery, { limit: 8, symbols })
        : getLatestNews({ limit: 6, symbols }),
      ...
      ...history,
      {
        ...
        content: [
          ...
          news
            ...
            .map(
              (item) =>
                `${item.symbol || "MARKET"} [${item.severity}]: ${item.title}`,
            )
            .join("\n") || "No news supplied.",
