...
Ollama should run on a persistent backend host such as EC2 or ECS/Fargate, not inside Lambda. Lambda remains the orchestration layer. News is wired to CryptoCompare's public news endpoint by default via `NEWS_FEED_URL`; production usage may need an API key or paid tier depending on provider limits. Email notifications through SES also require verified identities and may require moving the AWS account out of the SES sandbox.
